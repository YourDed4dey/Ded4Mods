...
using DedsBosses.Common.Global;
...
using DedsBosses.Content.Items.Drops.VoiyedDrops.VoiyedBossBag;
using DedsBosses.Content.Items.Drops.VoiyedDrops.VoiyedRelic;
using DedsBosses.Content.Items.Drops.VoiyedDrops.VoiyedTrophy;
using DedsBosses.Content.Items.Armor.Vanity.BossMasks;
using DedsBosses.Content.Items.Pets.VoiyedPet;
...
namespace DedsBosses.Content.NPCs.Bosses.VoiyedBoss
{
    ...
    public class Voiyed : ModNPC
    {
        ...
        private bool secondStageDamage;
        ...
        public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "DedsBosses/Assets/Textures/Bestiary/VoiyedBoss_Preview",
        ...
        public override void SetDefaults()
        {
            ...
            NPC.boss = true;
            NPC.value = Item.buyPrice(platinum:0, gold:15, silver:0, copper:0); //Golem and Plnatera drop 15 gold.
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            ...
            bestiaryEntry.Info.AddRange(new List<IBestiaryInfoElement> {
                ...
                new FlavorTextBestiaryInfoElement("A celestial being with the capability of collapsing the world at his will. He is here to seek a fight with a powerful warrior.")
            });
        }
        ...
        /*public override void ScaleExpertStats(int numPlayers, float bossLifeScale)
        {
            ...
            Expert Mode:
            - Boss does 125 damage in Stage 1 - Boss has 52850 hp //71347 on 2 players (1.35 is multiplier for each player)
            - Boss does 187 damage in Stage 2
            
            Master Mode:
            - Boss does 188 damage in Stage 1 - Boss has 79275 hp //107021 on 2 players (1.35 is multiplier for each player)
            - Boss does 282 damage in Stage 2
            
            //DMG MULTIPLIER OF FTW = 33.33% (or .3333)

             FTW Expert Mode:
            - Boss does 125 damage in Stage 1 - Boss has 52850 hp //71347 on 2 players (1.35 is multiplier for each player)
            - Boss does 250 damage in Stage 2
            
             FTW Master Mode:
            - Boss does 188 damage in Stage 1 - Boss has 79275 hp //107021 on 2 players (1.35 is multiplier for each player)
            - Boss does 376 damage in Stage 2
            
             FTW Legendary Mode:
            - Boss does 336 damage in Stage 1 - Boss has 105700 hp //142695 on 2 players (1.35 is multiplier for each player)
            - Boss does 672 damage in Stage 2*/
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<VoiyedTrophy>(), 10));

            notExpertRule.OnSuccess(ItemDropRule.Common(ModContent.ItemType<VoiyedMask>(), 7));

            notExpertRule.OnSuccess(ItemDropRule.Common(ModContent.ItemType<VoiyedShield>(), 1));

            npcLoot.Add(ItemDropRule.BossBag(ModContent.ItemType<VoiyedBossBag>()));

            npcLoot.Add(ItemDropRule.MasterModeCommonDrop(ModContent.ItemType<VoiyedRelic>()));

            npcLoot.Add(ItemDropRule.MasterModeDropOnAllPlayers(ModContent.ItemType<VoiyedPet>(), 4));

            npcLoot.Add(notExpertRule);

            if (ModContent.GetInstance<DedsBossesConfig>().VoiyedIsPartOfProgressionToggle)
            {
                npcLoot.Add(ItemDropRule.Common(ItemID.LihzahrdAltar, 1));
                return;
            }
        }
        ...
        public override void AI()
        {
            ...
            if (SecondStage)
            {
                ...
                if (aiTimer >= 60)
                {
                    NPC.velocity = Vector2.Normalize(bossToPlayer) * 15f; //Change depending on difficulty
                    aiTimer = 0;
                }
                ...
                if (preparingCelestialTeleport)
                {
                    ...
                    else
                    {
                        ...
                        Dust.NewDust(celestialTeleportPosition, NPC.width, NPC.height, DustID.RedTorch, Scale: 2f);
        ...
        private void TriggerMeteorShower()
        {
            ...
            if (NPC.life <= NPC.lifeMax * 0.9f)
            {
                ...
                for (int i = 0; i < meteorCount; i++)
                {
                    ...
                    int projectileIndex = Projectile.NewProjectile(null, spawnPosition, velocity, projectileType, 15, 0f, Main.myPlayer);
                    ...
                    if (projectileIndex != Main.maxProjectiles)
                    {
                        ...
                        projectile.GetGlobalProjectile<GlobalProjectiles>().meteorExplosionDelegate = ExplodeMeteor;
        ...
        private void ExplodeMeteor(Projectile projectile)
        {
            ...
            int explosion = Projectile.NewProjectile(null, explosionPosition, Vector2.Zero, ProjectileID.DD2ExplosiveTrapT3Explosion, 30, 0f, Main.myPlayer);
                    ...
                    else
                    {
                        ...
                        if (bloodRainTimer % 30 == 0) // Spawn a new projectile every 30 ticks (half a second)
                        {
                            for (int i = 0; i < 20; i++)
                            {
                                ...
                                // Get the player's Y position
                                //float playerY = NPC.Center.Y;

                                // Calculate the spawn position above the player off the screen
                                //Vector2 spawnPosition = new Vector2(NPC.Center.X + Main.screenWidth / 2f * (float)Math.Cos(angle), playerY - Main.screenHeight + 500f);
                                Vector2 spawnPosition = new Vector2(NPC.Center.X, NPC.Center.Y - 1000f);

                                Vector2 velocity = new Vector2(Main.rand.NextFloat(-3f, 3f), Main.rand.NextFloat(10f, 15f));

                                // Spawn the meteor projectile
                                int projectileIndex = Projectile.NewProjectile(null, spawnPosition, velocity, ProjectileID.BloodRain, 15, 0f, Main.myPlayer);
