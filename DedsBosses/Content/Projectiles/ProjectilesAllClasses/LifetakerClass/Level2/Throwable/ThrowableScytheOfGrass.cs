...
{
    public class ThrowableScytheOfGrass : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // Set the poison chance percentage (e.g., 20% chance to poison)
            int poisonChance = 25; // Adjust this value as needed

            // If the random number is less than or equal to the poison chance, apply poison
            if (chance <= poisonChance)
            {
                target.AddBuff(BuffID.Poisoned, 300); // Apply Poisoned buff for 5 seconds (300 frames)
            }
        }
