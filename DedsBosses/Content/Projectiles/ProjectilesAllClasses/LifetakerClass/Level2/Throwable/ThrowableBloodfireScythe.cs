...
{
    public class ThrowableBloodfireScythe : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (target.life <= 0)
            {
                int lifeGained = Main.rand.Next(1, 4);
                player.statLife += lifeGained;
                player.HealEffect(lifeGained); // Show healing effect
            }
        }
        int timer;
