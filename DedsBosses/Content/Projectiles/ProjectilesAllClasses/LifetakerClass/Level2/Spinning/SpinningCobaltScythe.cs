...
{
    public class SpinningCobaltScythe : ModProjectile
    {
        private int timer = 0;
        private const int maxTimer = 10; // Adjust this value to change the duration (10 = 1/6 second)
        ...
        public override void AI()
        {
            ...
            // Check if the timer has exceeded the allowed duration
            if (timer > maxTimer)
            {
                canParry = false;
            }

            // Check for collisions with enemies only during the first second
            if (timer <= maxTimer)
            {
                // Check for collisions with NPCs or other projectiles
                for (int i = 0; i < Main.npc.Length; i++)
                {
                    NPC npc = Main.npc[i];
                    if (npc.active && !npc.friendly && npc.type != NPCID.TargetDummy && canParry == true)
                    {
                        if (Projectile.Hitbox.Intersects(npc.Hitbox))
                        {
                            // Reduce damage taken by the owner
                            int damageReduction = 10; // Adjust the amount to reduce damage
                            int damage = 0; // Calculate or set the damage taken here
                            ...
                            for (int j = 0; j < 1; j++)
                            {
                                Dust.NewDust(player.position, player.width, player.height, DustID.YellowTorch, Scale: 1f);
                            }

                            // Apply the damage reduction to the owner
                            Main.player[Projectile.owner].Hurt(PlayerDeathReason.ByProjectile(Projectile.whoAmI, damage), damage, 0);
                            //Projectile.Kill();
                            return;
                        }
                    }
                ...
                for (int i = 0; i < Main.projectile.Length; i++)
                {
                    ...
                    if (projectile.active && !projectile.friendly && canParry == true)
                    {
