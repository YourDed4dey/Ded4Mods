...
{
    public class SeedShieldDash : ModPlayer
    {
            ...
            if (DashTimer > 0)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (rectangle.Intersects(rect) && (nPC.noTileCollide || Player.CanHit(nPC)))
                    {
                        ...
                        int type = Projectile.NewProjectile(null, position, direction * speed, ProjectileID.SeedlerNut, (int)(num), 0, Main.myPlayer);
                        Main.projectile[type].hostile = false;
                        Main.projectile[type].friendly = true;
                        Main.projectile[type].penetrate = 3;
