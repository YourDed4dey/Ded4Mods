...
{
    public class EnchantedShieldDash : ModPlayer
    {
        ...
        public const float DashVelocity = 13f; //5-9 is slow, 10-16 is medium,17-22 is fast, 23-30 is insanely fast, above that is TOO fast
            ...
            if (DashTimer > 0)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (rectangle.Intersects(rect) && (nPC.noTileCollide || Player.CanHit(nPC)))
                    {
                        ...
                        int type = Projectile.NewProjectile(null, position, direction * speed, ProjectileID.EnchantedBeam, (int)(num), 0, Main.myPlayer);
                        Main.projectile[type].hostile = false;
                        Main.projectile[type].friendly = true;
                        Main.projectile[type].penetrate = -1;
                        //nPC.AddBuff(type: BuffID.OnFire, time: 60 * 5); //This adds the buff to the npc when hitting an NPC
                    }
                }
