...
namespace RuinMod.Content.Weapons.ShieldClassWeapons.PreHardmode.BeeShield
{
    public class BeeShieldDash : ModPlayer
    {
            ...
            if (DashTimer > 0)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (rectangle.Intersects(rect) && (nPC.noTileCollide || Player.CanHit(nPC)))
                    {
                        ...
                        int type = Projectile.NewProjectile(null, position, direction * speed, ProjectileID.Bee, (int)(num), 0, Main.myPlayer);
                        Main.projectile[type].hostile = false;
                        Main.projectile[type].friendly = true;
                        //NPC.NewNPC(nPC.GetSource_FromAI(), X, Y, Proj, 5); //Summons NPCs
