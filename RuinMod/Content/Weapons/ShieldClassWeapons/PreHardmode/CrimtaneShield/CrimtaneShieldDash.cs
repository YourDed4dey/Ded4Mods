...
{
    public class CrimtaneShieldDash : ModPlayer
    {
        ...
        public const float DashVelocity = 14f; //5-9 is slow, 10-16 is medium,17-22 is fast, 23-30 is insanely fast, above that is TOO fast
            ...
            if (DashTimer > 0)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (rectangle.Intersects(rect) && (nPC.noTileCollide || Player.CanHit(nPC)))
                    {
                        ...
                        Player.eocHit = i;
                        nPC.AddBuff(type: ModContent.BuffType<BloodButcheredDebuff>(), time: 60 * 5); //This adds the buff to the npc when hitting an NPC
                    }
                }
