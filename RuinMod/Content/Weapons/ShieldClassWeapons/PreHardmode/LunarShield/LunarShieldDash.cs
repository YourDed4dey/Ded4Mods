...
{
    public class LunarShieldDash : ModPlayer
    {
        ...
        public const float DashVelocity = 18f; //5-9 is slow, 10-16 is medium,17-22 is fast, 23-30 is insanely fast, above that is TOO fast
        ...
        public override void PreUpdateMovement()
        {
            ...
            if (DashTimer > 0)
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                    ...
                    {
                        float num = 37f * shieldDamage;//Player.GetCritChance<ShieldClassDamage>(); //38f here is the dmg
                        float num2 = 9f;
                        bool crit = false;
                        if (Player.kbGlove)
                        {
                        ...
                        if (Main.rand.Next(100) < shieldCrit)//Player.GetCritChance<ShieldClassDamage>())
                        {
                            crit = true;
                        }
                        ...
                        //nPC.AddBuff(type: ModContent.BuffType<CorruptedDebuff>(), time: 60 * 5); //This adds the buff to the npc when hitting an NPC
                    }
                }
