...
namespace RuinMod.Content.Potions.Debuffs.BloodButchered
{
    internal class BloodButcheredDebuff : ModBuff
    {
        public int lifeRegenExpectedLossPerSecond = -1;
        public override void SetStaticDefaults()
        {
            //DisplayName.SetDefault("Blood Butchered");
        ...
        public override void Update(NPC npc, ref int buffIndex)
        {
            ...
            if (Main.rand.Next(4) < 3)
            {
                Dust dust18 = Dust.NewDustDirect(new Vector2(npc.position.X - 2f, npc.position.Y - 2f), npc.width + 4, npc.height + 4, DustID.Blood, npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 1.5f);
                dust18.noGravity = true;
                dust18.velocity.X = 1.8f;
                dust18.velocity.Y -= 0.5f;
                if (Main.rand.NextBool(4))
