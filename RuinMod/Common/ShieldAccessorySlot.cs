...
using RuinMod.Content.Weapons.ShieldClassWeapons;
...
{
    /*public class ShieldAccessorySlot : ModAccessorySlot
    {
    ...
    */
    public class ShieldSlot : ModAccessorySlot
    {
        ...
        {
            if (checkItem.shieldSlot > 0) // if is shield, then can go in slot
                return true;
        ...
        {
            if (item.shieldSlot > 0) // If is shield, then we want to prioritize it to go in to our slot.
                return true;
        ...
        public override string FunctionalTexture => "Terraria/Images/Item_" + ItemID.CobaltShield;
