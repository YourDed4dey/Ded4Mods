...
namespace RuinMod.Common.Global.DevastatedDiff.NPCS.BossAI.EOW
{
    internal class EOWHeadOverride : GlobalNPC
    {
        ...
        public static bool BuffedWormAI(NPC npc)
        {
            if (RuinWorld.devastated)
            {
                int type = npc.type;
                ...
                if (type < 13 || type > 15 || type != 13 && (type == 13 || Main.npc[(int)npc.ai[1]].alpha >= 85))
                {
                    //return;
                }
                if (alpha > 0 && npc.life > 0)
                {
                    ...
                    {
                        //int num81 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), width, height, DustID.CursedTorch, 0f, 0f, 100, default(Color), 2f);
