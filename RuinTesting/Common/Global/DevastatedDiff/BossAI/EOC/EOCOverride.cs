...
public class EOCOverride : GlobalNPC
{
    ...
    public void PostAINPC(NPC npc)
    {
        ...
        {
            case 0:
                DoBehavior_Hover(npc, player, ref attackTimer, lifeRatio);
                break;
            case 1:
                DoBehavior_HoverCharge(npc, player, false, lifeRatio, ref attackTimer);
                break;
        }
        if (!Spawnedminions && !NPC.AnyNPCs(ModContent.NPCType<EOCServant>()))
        {
            SpawnMinions(npc);
            Spawnedminions = true;
        }
    }
    public static void DoBehavior_Hover(NPC npc, Player target, ref float attackTimer, float lifeRatio)
    {
    ...
    public static void DoBehavior_HoverCharge(NPC npc, Player target, bool phase2, float lifeRatio, ref float attackTimer)
    {
        int hoverTime = 60;
        int chargeTime = 45;
        float chargeSpeed = phase2 ? MathHelper.Lerp(9f, 12.3f, 1f - lifeRatio) : MathHelper.Lerp(8f, 10.2f, 1f - lifeRatio);
        if (phase2)
        {
            chargeSpeed += 1.5f;
            chargeTime -= 15;
    ...
    public static bool BuffedEyeofCthulhuAI(NPC npc)
    {
        ...
        else
        {
            ...
            else if (npc.ai[1] == 5.0)
            {
                ...
                Vector2 destination = Main.player[npc.target].Center + Vector2.UnitY * num42;
                Vector2 desiredVelocity = npc.DirectionTo(destination) * num45;
                npc.SimpleFlyMovement(desiredVelocity, moveSpeed);
                ++npc.ai[2];
                if (npc.ai[2] % 45.0 == 0.0)
                {
    ...
    public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D texture2D = ModContent.Request<Texture2D>("RuinTesting/Assets/Textures/DevaDiff/Devastated/NPC_4_Deva").Value;
